...
\section[Továbbfejlesztés]{Korlátok és továbbfejlesztési lehetőségek}

\textbf{A projekt jelenlegi korlátjai:}
\begin{itemize}
	\item jelenleg nem egy kész játékról, csak prototípusról beszélünk;
	\item nincsennek végigjátszható pályák, vagy elérhető célok;
	\item a legtöbb modell a pályán primitív alakzatok, kezdetleges textúrákkal;
	\item a mozgásparamétereket jelenleg még finomhangolni kell, további játék tesztelést igényel.
\end{itemize}
...
\textbf{The project's limitations:}
\begin{itemize}
	\item so far it's not an actual game, more so a prototype;
	\item there are no levels that can be completed, or achievements that can be earned;
	\item most of the models use primitive shapes with placeholder textures;
	\item the movement system's parameters still need fine tuning and the gameplay requires further playtesting.
\end{itemize}
