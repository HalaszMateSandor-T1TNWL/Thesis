...
\section[Rigid vs Character]{RigidBody vs CharacterBody}
...
Az előzőekben elmondottakból tehát, a játékos színtere egyelőre így néz ki: egy CharacterBody3D Node mint a gyökér a fába, egy MeshInstance3D és CollisionShape3D mint gyerek Node-ok, lásd: \ref{BasicPlayerScene}.
...
A Node3D objektumok valójában pontok a háromdimenziós térben, így tökéletesen megfelelnek a különböző irányok nyílvántartására, ezért választot\-tam inkább ezt a megoldást, mint azt, hogy változókat hozzak létre ezek figyelésére. 
...
\section[Állapotok]{Az állapotok megvalósítása}
...
A megvalósítás során az állapotokat egy időben írtam a játékos szkriptjével. Létrehoztam egy Player szkriptet és osztályt, amit a CharacterBody3D Node-hoz csatoltam. Ezen az osztályon belül hoztam létre azokat a metódusokat, amelyeket az állapotátmenetek vizsgálására használtam. Vala\-mint itt számítottam ki a kamera mutatási irányát és a mozgásvektort is.\\
...
A játékos végső struktúrája a \ref{PlayerFinal} ábrán látható. A Node-ok sorrendje a fában nem számít, nem gyorsítja fel az elérést, csupán esztétika, egyedül a hierarchia fontos.

\begin{figure}
	\centering
	\includegraphics[width=\linewidth]{Images/PlayerSceneFinal.png}
	\caption{A végső játékos színtér}
	\label{PlayerFinal}
\end{figure}

\section[Új állapot Hozzáadása]{Az állapotgép bővítésének lépései}

Számos alkalommal említettem a rendszer könnyű bővíthetőségét és ideje, hogy bemutassam ennek a lépéseit:

\begin{minipage}{0.5\textwidth}
	\begin{figure}[H]
		\includegraphics[width=\textwidth]{Images/AddState_1.png}
		\caption{PlayerController jobb klikk menü}
		\label{AddState_1}
	\end{figure}
\end{minipage}
\hspace{0.05\textwidth}
\begin{minipage}{0.4\textwidth}
		Ahogy a \ref{AddState_1} ábra is mutatja, első lépésként a játékos színterén belül gurítsunk le a "PlayerController" Node-hoz és a jobb egérbombbal kattintsunk rá és vállasszuk ki az "Add Child Node" opciót (vagy a PlayerController kiválasztása után nyomjuk le a gyorsbillentyűt: Ctrl + A).
\end{minipage}

\vspace{0.5cm}

A \ref{AddState_2} ábrán látható menü felgurik és itt az alap "Node" nevű, objektumot kell kiválasztani. Ha nem látható a listán, akkor a felső "Search" mezőbe beírva lehet rákeresni. Erre duplán rákattintva, vagy a bal alsó sarokban lévő "Create" gombra kattintva adhatjuk hozzá a hierarchiához.

\begin{figure}[H]
	\includegraphics[width=\textwidth]{Images/AddState_2.png}
	\caption{Node választó menü}
	\label{AddState_2}
\end{figure}
...
\begin{minipage}{0.4\textwidth}
	Navigáljunk a fában az újonnan létrehozott Node-hoz és nevezzük át valami nekünk megfelelő névre vagy jobb-klikkelve a Node-ra és kiválasztva a "Rename" opciót, vagy használva a gyorsbillentyűt: F2, ahogy az a \ref{AddState_3} ábrán is látható.
\end{minipage}
\hspace{0.05\textwidth}
\begin{minipage}{0.5\textwidth}
	\begin{figure}[H]
		\includegraphics[width=\textwidth]{Images/AddState_3.png}
		\caption{Jobb-klikk menü az új Node-on}
		\label{AddState_3}
	\end{figure}
\end{minipage}
...
Ezek után nyissuk meg ismét a jobb-klikk menüt és kattintsunk az "Attack Script" gombra, ahogy a \ref{AddState_4} ábra is mutatja, majd a felugró ablakban, amit a \ref{AddState_4,5} ábra mutat, hagyjuk a fájl nevét az automatikusan generáltan, mivel fontos, hogy mind a szkript neve és az osztály neve ugyan az legyen. A fájl elérési útját nyugodtan átírhatjuk. Végül a "Create" gombra kattintva létrehoz a megadott elérési úton egy szkriptet és hozzácsatolja a Node-hoz.

\vspace{0.5cm}

\begin{minipage}{0.5\linewidth}
	\begin{figure}[H]
		\includegraphics[width=\textwidth]{Images/AddState_4.png}
		\caption{A Godot szkriptszerkesztője}
		\label{AddState_4}
	\end{figure}
\end{minipage}
\hspace{0.05\textwidth}
\begin{minipage}{0.5\linewidth}
	\begin{figure}[H]
		\includegraphics[width=\textwidth]{Images/AddState_4,5.png}
		\caption{A Node leszármaztatást megváltoztattuk State-re}
		\label{AddState_4,5}
	\end{figure}
\end{minipage}
...
A Godot kezelőfelülete át fog váltani "Script" módba, ahogy a \ref{AddState_5} ábrán látható. A Godot szkriptszerkesztőjébe alapvetően nincs C\#-hoz LSP, viszont van szintaxis kiemelés. A szerkesztőn belül a leszármaztatást meg kell változtatni "Node"-ról "State"-re, mint a \ref{AddState_5,5} ábrán. Ha visszaemlékeznek a State osztályt direkt ezért hoztuk létre, hogy a abból származtassuk le az összes többi állapotot. Most már a PlayerController fel fogja ismerni és be fogja sorolni az elérhető állapotok közé.

\begin{figure}
	\includegraphics[width=\textwidth]{Images/AddState_5.png}
	\caption{Node jobb-klikk menü}
	\label{AddState_5}
\end{figure}

\begin{figure}[H]
	\includegraphics[width=\textwidth]{Images/AddState_5,5.png}
	\caption{Szkript hozzáadás menüje}
	\label{AddState_5,5}
\end{figure}

Jelenleg nem tudunk ebbe az állapotba eljutni, mivel nincs hozzákötve feltétel a Player osztályon belül, viszont a debug menüben fel lesz sorolva, ahogy azt a \ref{CoolNewState} ábra is mutatja.

\begin{figure}[H]
	\includegraphics[width=\linewidth]{Images/CoolNewState.png}
	\caption{Az új állapotunk felsorolva a meglévők mellett}
	\label{CoolNewState}
\end{figure}

Ez a dinamikus debug menü létrehozásnak köszönhető, amit a PlayerController osztályon belül valósítottam meg. A felhasználó felület Node-ját lekérem, majd átiterálok az összes meglévő állapoton és azoknak a nevét felhasználva hozok létre új CheckBox objektumokat, amiket elhelyezek a képernyőn egymás alá.
...
Amint látják 5 lépésben képesek voltunk hozzáadni egy teljesen új állapotot a rendszerünkhöz. Érdemes megjegyezni, hogy ha nem csináltam volna meg ezt a dinamikus debug menüt, akkor is létezne az állapot, egyszerűen nem lenne nyomon követve vizuálisan, lásd: \ref{CoolNewState_ToString} ábra.

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{Images/CoolNewState_ToString.png}
	\caption{Az új állapotunk kiiratva a konzolra}
	\label{CoolNewState_ToString}
\end{figure}